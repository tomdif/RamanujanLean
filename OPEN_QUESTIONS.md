...
- `Mod691.tau_congruence_mod691` — `(τ(n) : ZMod 691) = (σ₁₁(n) : ZMod 691)`, given that `τ(n)` and `[qⁿ]E₄³`
  are integers. The arithmetic (`691 ∣ 432000·(τ − σ)`, `691 ∤ 432000`) is fully discharged.

**Irreducible remainder — integrality of the `η²⁴` q-expansion (`τ(n) ∈ ℤ`).** Classified: *infrastructure
gap*. Mathlib has **no** integrality statement for `Δ`, `E₄`, `E₆` q-expansions. The natural route is the
proved `1728·τ(n) = [qⁿ]E₄³ − [qⁿ]E₆²` (`tau_smul_eq_coeff`): both right-hand coefficients are integer
combinations of `σ₃`, `σ₅`, so integrality of `τ` is exactly `1728 ∣ ([qⁿ]E₄³ − [qⁿ]E₆²)` — a concrete
elementary divisibility (`2⁶·3³`), the classical proof that `Δ = (E₄³−E₆²)/1728` has integer coefficients.
Building it is a self-contained number-theory project independent of everything above.
...
|---|---|---|
| mod-691 congruence | **proved** (conditional) | integrality `1728 ∣ [qⁿ](E₄³−E₆²)` — reachable NT project |
